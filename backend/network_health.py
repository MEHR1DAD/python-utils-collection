    ...
    def export_metrics(self, current_alerts, all_messages=[]):
        ...
        if all_messages:
            ...
            stop_words = set([
                ...
                'خواهد', 'داد', 'داریم', 'شما', 'ما', 'آنها', 'اگر', 'اما', 'ولی', 'چون', 'زیرا', 'وقتی', 'هنگام', 'هنگامی', 'قبل', 'بعد', 'پس', 'حتی', 'شاید', 'باید', 'حتما', 'دقیقا', 'واقعا', 'کشته', 'شدگان',
                'دیگر', 'پنجشنبه', 'جمعه', 'شنبه', 'یکشنبه', 'دوشنبه', 'سه‌شنبه', 'چهارشنبه'
            ])
            ...
            for msg in recent_msgs:
                ...
                for i in range(len(tokens)):
                    w1 = tokens[i]
                    if w1 not in BLOCKLIST:
                        unigram_counts[w1] = unigram_counts.get(w1, 0) + 1
                    
                    if i < len(tokens) - 1:
                        w2 = tokens[i+1]
                        bigram = f"{w1} {w2}"
                        if bigram not in BLOCKLIST and w1 not in BLOCKLIST and w2 not in BLOCKLIST:
